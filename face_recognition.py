...
if __name__ == "__main__":
    ...
    path_model = os.path.join(
        os.getcwd(), 'ai_model/model/trained_knn_model.clf')

    # print("Training KNN classifier...")
    # classifier = train(path_train,
    #                    model_save_path=path_model, n_neighbors=2)
    ...
    for image_file in os.listdir(path_test):
        ...
        predictions = predict(full_file_path, model_path=path_model)

        # Print results on the console
        for name, (top, right, bottom, left) in predictions:
            if name != 'unknown':
                image_file_new = str(time.time()) + \
                    os.path.splitext(image_file)[1]
                old_photo_path = os.path.join(
                    os.getcwd(), 'ai_model/test/'+image_file)
